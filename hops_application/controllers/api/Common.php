...
class Common {
	...
	public function single_push_notification_service($device_details,$message) {

		$this_obj =& get_instance();

		if($device_details['logs_device_type'] == 1) {

			// prepare the bundle
			$fields = array(
							'to' => $device_details['logs_device_token'], // single device token (string format)
		...
		else if($device_details['logs_device_type'] == 2) {
			...
			if(HOPS_ENVIRONMENT == "production") {

				$pemFile = (FCPATH . 'assets/pem/hops_pro.pem'); // For Production	
	...
	public function multiple_push_notification_service($device_details,$message) {
		...
		if(!empty($device_details['android'])) {
			...
			$android_users = $device_details['android'];

			foreach ($android_users as $user_key => $user_val) {
				...
				// prepare the bundle
				$fields = array(
								'to' => $user_val['logs_device_token'], // string values
								'data' => $message // array values
							);
		...
		if(!empty($device_details['ios'])) {
			...
			foreach ($ios_users as $user_key => $user_val) {
